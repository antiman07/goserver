...
func PrintPlayerOperator(begin time.Time,end time.Time){
	...
	for i := 1; i <= 4; i++{
		myrpc.Rpcqueue <- fmt.Sprintf("房间%d 弃牌次数:%d 跟注次数:%d 加注次数:%d 过牌次数:%d",i,
			roomOperList[i].Discardtimes,roomOperList[i].Cingltimes,roomOperList[i].Raisetimes,roomOperList[i].Passtimes)
	}
}

//统计所有机器人做的各种操作次数

func IncreAllPlayerData(p *Suoha_Player){
	...
	for i := 1; i <= 4; i++{
		roomOperList[i].Passtimes 	 += p.RoomOperList[i].Passtimes
