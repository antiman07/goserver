...
func (self *tcpSession) Start() {
	...
	// 将会话添加到管理器 
    //把这个 session 交给上层(self.Peer())的tcpAcceptor管理,管理变量是sesById sync.Map,已经实现多gortinue安全访问。
...
func newSession(conn net.Conn, p cellnet.Peer, endNotify func()) *tcpSession {
	self := &tcpSession{
		...
		pInterface: p,
		CoreProcBundle: p.(interface {        //临时转换一个接口并调用其方法
			GetBundle() *peer.CoreProcBundle
		}).GetBundle(),
	}

//CoreProcBundle变量的这种用法 和 chat\client\main.go ReadConsole 函数相似
	return self
}
